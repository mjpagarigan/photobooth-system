...
- [Frame data model](tickets/03-dynamic-infinite-frames.md): hardcoded 2-tuple replaced by ordered array-backed store with CRUD IPC; existing mat/anniversary frames seeded as entries 1–2; slot count stays 3 (photo count unchanged).
...
- Frame deletion policy when historical sessions reference a deleted frame (compositing archived collages).
...
- `PHOTO_COUNT = 3` and 3-slot strip geometry — unchanged.
